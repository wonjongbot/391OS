...
//  printf("CURRENT PCB ADDR: %x\n", current);
    if (fd >= 8 || fd < 0) return -1;    //0=<fd<8
    if (PCB(terminals[sched_terminal_idx].pid)->filearray[fd].flags == 0) return -1;
    return PCB(terminals[sched_terminal_idx].pid)->filearray[fd].ops->write(fd, buf, nbytes);
/*
