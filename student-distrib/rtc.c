...
/* Global Variables */
unsigned rtc_counter[3] = {0, 0, 0};   // Keeps track of number of interrupts generated by RTC
...
int32_t rtc_set_freq(unsigned frequency){
    if(frequency == 0){
        ...
        // set counter target
        rtc_target[current_terminal] = (float)(1024) / (float)frequency;  // Gives number of interrupts. 1024 Hz
    }
...
void rtc_handler(void) {
    ...
    rtc_counter[0] = (rtc_counter[0] + 1)%rtc_target[0];
    rtc_counter[1] = (rtc_counter[1] + 1)%rtc_target[1];
    rtc_counter[2] = (rtc_counter[2] + 1)%rtc_target[2];
    send_eoi(RTC_IRQ);
    restore_flags(flags);
    return;
}
...
int32_t rtc_read (int32_t fd, void* buf, int32_t nbytes) {
    ...
    while (interrupt_flag[current_terminal] == 0);
    ...
    interrupt_flag[current_terminal] = 0; // interrupt is off
    return 0;
}
