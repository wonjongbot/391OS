...
unsigned rtc_target = 1;    // To keep track if counter > this value
int interrupt_flag = 0;     // interrupt is off
...
int32_t rtc_read (int32_t fd, void* buf, int32_t nbytes) {
    int interrupt_flag = 1; // interrupt is set
    while (interrupt_flag == 0) {
        continue;   // wait until interrupt handler clears flag
    }
...
int32_t rtc_write (int32_t fd, const void* buf, int32_t nbytes) {
    ...
    return rtc_set_freq(*(int32_t*)buf);    // buf is the new frequency to be written
